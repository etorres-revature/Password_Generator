...
function generatePassword() {
  ...
  for (let i = 0; i < passwordLength; i++) {
    newPassword =
      newPassword +
      strChoices.charAt(Math.floor(Math.random() * strChoices.length));
  }
  return newPassword;
}
...
function userPrompts() {
  ...
  if (isNaN(passwordLength) || passwordLength < 8 || passwordLength > 128) {
    ...
    let hasUpper = confirm(
      "Would you like for your password to contain upper case letters?"
    );
    if (hasUpper) {
    ...
    if (hasSpecial) {
      ...
      alert(
        "you have not chosen any characters with which to make your password"
      );
    }
